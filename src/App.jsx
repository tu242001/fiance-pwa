...
export default function App() {
  ...
  // Force sync from cloud (used by pull-to-refresh)
  const syncNow = async () => {
    if (!user || pullState==='refreshing') return
    ...
    } catch(e) { setSyncErr('同步失敗') }
    setPullState('idle'); setPullY(0)
  }
  ...
  const onTouchMove = (e) => {
    ...
    const el = scrollRef.current
    if (el && el.scrollTop > 0) { pullRef.current.pulling = false; setPullY(0); setPullState('idle'); return }
    const dy = e.touches[0].clientY - pullRef.current.startY
    if (dy <= 0) return
    // Rubber band: diminishing returns after threshold
    const clamped = dy > PULL_THRESHOLD ? PULL_THRESHOLD + (dy - PULL_THRESHOLD) * 0.3 : dy
    setPullY(Math.min(clamped, PULL_THRESHOLD + 30))
    setPullState(dy >= PULL_THRESHOLD ? 'ready' : 'pulling')
  ...
  const handleImageScan = async (file) => {
    if (!file) return
    if (!geminiKey) { setModal('setKey'); return }
    setScanLoading(true); setScanResult(null); setScanSelected([]); setScanErr('')
    ...
    reader.onload = async (e) => {
      ...
      try {
        const raw = await callGemini(
          `請分析這張截圖（可能是信用卡帳單、訂閱服務頁面、消費記錄等），提取所有消費或訂閱項目。\n\n請以 JSON 陣列格式回傳，格式如下，只回傳 JSON 不要其他任何文字或說明：\n[\n  {\n    "desc": "商品或服務名稱（字串）",\n    "amount": 123,\n    "cat": "餐飲",\n    "isRecurring": false,\n    "billingDay": null,\n    "date": "${nowDate()}"\n  }\n]\n\ncat 只能從以下選一：餐飲、購物、交通、娛樂、日用、旅遊、訂閱、其他\n若是每月固定訂閱（Netflix/Spotify 等）則 isRecurring 設 true 並填 billingDay（數字）\n金額用新台幣整數\n若截圖無任何消費項目，回傳空陣列 []`,
          base64, file.type || 'image/jpeg'
        )
        // Robust JSON extraction: find first '[' to last ']'
        const start = raw.indexOf('[')
        const end = raw.lastIndexOf(']')
        if (start === -1 || end === -1) throw new Error('Gemini 未回傳 JSON 格式，請重試')
        const jsonStr = raw.slice(start, end + 1)
        const parsed = JSON.parse(jsonStr)
        if (!Array.isArray(parsed)) throw new Error('回傳格式錯誤')
        setScanResult(parsed)
        setScanSelected(parsed.map((_, i) => i))
      } catch (err) {
        setScanResult([])
        setScanErr(err.message || '辨識失敗，請確認 API Key 是否正確，或換一張更清晰的截圖')
      }
      setScanLoading(false)
    }
    reader.onerror = () => {
      setScanErr('圖片讀取失敗，請重試')
      setScanLoading(false)
    }
  ...
  const importScanned = () => {
    ...
    // Duplicate detection: same desc + amount + date already exists
    const isDupTx = (r) => txs.some(t =>
      t.type === 'expense' &&
      t.desc === r.desc &&
      t.amt === r.amount &&
      t.date === r.date
    )
    // Duplicate sub: same name already exists
    const isDupSub = (r) => subs.some(x =>
      x.name.trim().toLowerCase() === r.desc.trim().toLowerCase()
    )

    const newTxs = toImp
      .filter(r => !r.isRecurring)
      .filter(r => !isDupTx(r))
      .map(r => ({id:uid(), type:'expense', cid:cards[0]?.id||'', amt:r.amount, bAmt:r.amount, cat:r.cat, desc:r.desc, date:r.date}))

    const newSubs = toImp
      .filter(r => r.isRecurring)
      .filter(r => !isDupSub(r))
      .map(r => ({id:uid(), name:r.desc, amount:r.amount, cardId:cards[0]?.id||'', cat:'串流', day:r.billingDay||1}))

    const dupCount = toImp.length - newTxs.length - newSubs.length

    if (newTxs.length) updTxs([...newTxs, ...txs])
    if (newSubs.length) updSubs([...subs, ...newSubs])

    const msg = []
    if (newTxs.length + newSubs.length > 0) msg.push(`✅ 已匯入 ${newTxs.length + newSubs.length} 筆`)
    if (dupCount > 0) msg.push(`⚠️ ${dupCount} 筆重複略過`)
    if (msg.length) alert(msg.join('，'))

    setScanResult(null); setScanPreview(null); setScanSelected([]); setScanErr('')
    setPage('home')
  }
  ...
  const AiPage = () => (
    <div>
      ...
      {aiTab==='scan'&&(
        <div>
          <input ref={imgRef} type="file" accept="image/*" style={{display:'none'}} onChange={e=>{if(e.target.files[0])handleImageScan(e.target.files[0])}}/>
          {!scanResult&&!scanLoading&&(
            <div>
              <button onClick={()=>{ setScanErr(''); imgRef.current?.click() }} disabled={!geminiKey}
              ...
              {scanErr&&(
                <div style={{marginTop:12,padding:'10px 12px',background:'#E24B4A15',borderRadius:'var(--radius)',border:'0.5px solid #E24B4A44'}}>
                  <div style={{fontSize:13,color:'#E24B4A',fontWeight:500,marginBottom:4}}>辨識失敗</div>
                  <div style={{fontSize:12,color:'var(--text2)'}}>{scanErr}</div>
                </div>
              )}
          ...
          {scanResult&&!scanLoading&&(
            <div>
              ...
              {scanResult.length===0?(
                <div>
                  <div style={{fontSize:13,color:'var(--text2)',textAlign:'center',padding:'12px 0'}}>截圖中未發現消費項目</div>
                  {scanErr&&<div style={{fontSize:12,color:'#E24B4A',background:'#E24B4A15',padding:'8px 12px',borderRadius:'var(--radius)',marginBottom:10}}>{scanErr}</div>}
                  <button onClick={()=>{setScanResult(null);setScanPreview(null);setScanErr('')}} style={{width:'100%',padding:'9px',border:'0.5px solid var(--border2)',borderRadius:'var(--radius)',background:'none',cursor:'pointer',fontSize:13,color:'var(--text2)'}}>重新上傳</button>
                </div>
              ):(
                <>
                  ...
                  <div style={{display:'flex',gap:8,marginTop:8}}>
                    <button onClick={()=>{setScanResult(null);setScanPreview(null);setScanSelected([]);setScanErr('')}} style={{flex:1,padding:'9px',border:'0.5px solid var(--border2)',borderRadius:'var(--radius)',background:'none',cursor:'pointer',fontSize:13,color:'var(--text2)'}}>重新掃描</button>
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{
          position:'sticky', top:0, zIndex:20, overflow:'hidden', height:0,
          transition: pullState==='idle' ? 'height 0.25s ease' : 'none',
          ...(pullY > 0 || pullState==='refreshing' ? {height: pullState==='refreshing' ? 52 : pullY * 0.65} : {})
        }}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'10px 0',color:'var(--text2)',fontSize:13,height:52}}>
            <span style={{
              display:'inline-block', fontSize:18,
              transition:'transform 0.2s',
              transform: pullState==='refreshing' ? 'rotate(360deg)' : pullState==='ready' ? 'rotate(180deg)' : 'rotate(0deg)',
              animation: pullState==='refreshing' ? 'spin 0.8s linear infinite' : 'none'
            }}>
              {pullState==='refreshing' ? '◌' : pullState==='ready' ? '↑' : '↓'}
            </span>
            <span>{pullState==='refreshing' ? '同步中...' : pullState==='ready' ? '放開以同步' : '下拉重新整理'}</span>
          </div>
        </div>
